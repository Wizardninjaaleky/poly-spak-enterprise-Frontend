...
export const getOrderPayment = async (req, res) => {
  try {
    ...
    const payment = await getPaymentDetails(req.params.orderId); // Assuming getPaymentDetails fetches from Payment model
...
export const getPaymentHistory = async (req, res) => {
  try {
    const orders = await Order.find({ userId: req.user.id }) // Assuming userId field
...
export const verifySubmittedPayment = async (req, res) => {
...
export const getPayments = async (req, res) => {
  try {
    ...
    const payments = await Payment.find(); // Simplified to fetch all from model directly
...
export const getPaymentStats = async (req, res) => {
  try {
    // Simplified to fetch from model directly
    const totalPayments = await Payment.countDocuments();
    const totalAmount = await Payment.aggregate([{ $group: { _id: null, total: { $sum: '$amount' } } }]);

    const stats = { totalPayments, totalAmount: totalAmount.length > 0 ? totalAmount[0].total : 0 };
