...
const categories = [
  {
    name: 'Automotive',
    description: 'Car accessories, jump starters, and automotive tools',
    slug: 'automotive',
  },
  {
    name: 'Packaging',
    description: 'Garbage bags, seedling bags, and packaging materials',
    slug: 'packaging',
  },
];

// New products with proper structure
const newProductsData = [
  {
    name: 'High Power Digital Jump Starter with Compressor & Flashlight',
    description: 'Multifunctional car jump starter and air pump kit. Includes digital compressor with pressure gauge, adjustable flashlight, jumper cables, and carrying case. Perfect for emergency roadside assistance. Features LED display, multiple safety protections, and USB charging ports. Battery capacity: 68800mAh.',
    price: 9499,
  ...
  },
  {
    ...
    description: 'High-quality heavy-duty garbage bags suitable for home, office, and industrial use. Available in multiple sizes. Extra thick, leak-proof, and tear-resistant. Black color for discreet waste disposal. Roll of bags.',
    price: 1200,
...
async function uploadProducts() {
  try {
    ...
    for (const productData of newProductsData) {
      ...
      productFields.category = categoryMap[categoryName];
    ...
    const allProducts = await Product.find()
      ...
      .select('name price category inStock sku');
    
    allProducts.forEach((p, i) => {
      const catName = p.category?.name || 'No Category';
      console.log(`${i + 1}. ${p.name} - KSh ${p.price} (${catName}) [${p.sku}] ${p.inStock ? '✅' : '❌'}`);
    });
